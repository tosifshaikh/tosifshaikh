...
optColor["EDITED LOV VALUE"]="#66CCCC";
optColor["DELETED LOV VALUE"]="#99CCFF"; 
optColor["REORDERED LOV VALUE"]="#REORDER";
...
{ 
		...
		if(opt!="" || from_date!="" || to_date!="" || filtertype!=""  || txtold_status!="" || txtnew_status!="" || keyword!="")
		{
			 ...
			 if(clientId!="")
			 {
				 var clientId1="&clientId="+clientId;
				 strsting=strsting+clientId1 
			 }
			 if(typ!="")
			 {
				var typ1="&type="+typ;
		...
		var strURL = '';	
	    strURL="manage_audit_trail.php?&adt=WPH_Lov_Audit&act=MCC_ADT_XML&Start="+hdnStart+strsting;
			...
			if (req.readyState == 4) 
			{
				if (req.status == 200) 
				{
					...
					}catch(err)
					{
					    alert(err);
						getLoadingMain(0);
						ExceptionMsg("Section:MCC Work Pack History Edit/Reorder Lov Audit Trail - Error description: , "+err.message+" OR InValid XML (XML Parser Error).Error on cs_audit_trail_xml.php page.\n\n");
...
function Render_Grid(ArrHeader,optColor)
{
	...
	{		
		...
		{	
			...
			table +='<td nowrap="nowrap">'+clientName +'</td>';
			...
			if(old_value_arr.length>0 && old_value_arr[0]!="-")
			{
				if(old_value_arr[0])
				{
					Tampstr='<b>Column Name</b>:'+old_value_arr[0];
				}
				if(old_value_arr[1])
				{
					Tampstr=Tampstr+',<b> List Of Value</b>:['+old_value_arr[1]+']';
				}
				if(old_value_arr[2])
				{
					Tampstr=Tampstr+',<b> Status</b>:['+old_value_arr[2]+']';
				}
			}
			
			var Tampstr1="-";
			if(New_value_arr.length>0 && New_value_arr[0]!="-")
			{
				if(New_value_arr[0])
				{
					Tampstr1='<b>Column Name</b>:['+New_value_arr[0]+']';
				}
				if(New_value_arr[1])
				{
					Tampstr1=Tampstr1+',<b> List Of Value</b>:['+New_value_arr[1]+']';
				}
				if(New_value_arr[2])
				{
					Tampstr1=Tampstr1+',<b> Status</b>:['+New_value_arr[2]+']';
				}
			}
...
function export_xls()
{
	...
	window.open("export_audit_trail.php?hidqry="+qry+"&hidwca="+wca+"&hidExp_Type=MCC_WPH_Lov&Start="+hdnStart+strsting,"mccwindow","","false"); 	 
