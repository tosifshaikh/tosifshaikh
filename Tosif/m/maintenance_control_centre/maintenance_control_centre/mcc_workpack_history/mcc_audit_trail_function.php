...
//Function Use to Generate Xls Using PHP EXCEL Class.
	function MCC_WPH_ExportExcel()
	{
		...
		$operationArr1=array('COLUMN ADDED'=>"#99CC00",'COLUMN EDITED'=>"#FF9900",'COLUMN DELETED'=>"#FFFF00","COLUMN REORDERED"=>"#6666FF");//for Cs row wise.  
		$imagessrc="./images/".$_SESSION['css_path']."/logo.png";
		 ...
		 {      
				 if($db->query_mcc_others($part_sql,$WhereClauseArr,$Request,$eu))
				 {
						...
						$title="Workpack » Manage Workpack History Status List » Audit Trail";
						...
						{    
							 while($db->next_record())
							{
								...
								$clientName=getAirlinesNameById($db->f('client_id'));
								...
								if($old_value_arr[0])
								{
									$oldValStr.=$old_value_arr[0];
								}
								if($old_value_arr[1])
								{
									$oldValStr.=', Set Column Field Type: ['.$old_value_arr[1].']';
								}
								if($old_value_arr[2])
								{
									$oldValStr.=', Inserted Values: ['.$old_value_arr[2].']';
								}
								if($old_value_arr[3])
								{
									$oldValStr.=', Auto Prepare List: ['.$old_value_arr[3].']';
								}
								...
								if($New_value_arr[0])
								{
									$newValStr.=$New_value_arr[0];
								}
								if($New_value_arr[1])
								{
									$newValStr.=', Set Column Field Type: ['.$New_value_arr[1].']';
								}
								if($New_value_arr[2])
								{
									$newValStr.=', Inserted Values: ['.$New_value_arr[2].']';
								}
								if($New_value_arr[3])
								{
									$newValStr.=', Auto Prepare List: ['.$New_value_arr[3].']';
								}
