...
{ 
		...
		HdTitle=document.getElementById("HdTitle").value;
    ...
    var strURL="manage_audit_trail.php?HdTitle="+HdTitle+"&adt=MCC_ADT&act=MCC_ADT_XML&parentId="+parentId+"&TabId="+TabId+"&RecId="+RecId+"&pmo_flag="+pmo_flag+"&type="+Type+"&Start="+hdnStart+strsting;
	...
	{		
		...
		{	
			...
			table += '<tr name="TrId" id="TrId'+i+'" bgcolor="'+trClass+'" >';
					...
					table +='<td>'+title+'</td>';
					if(title.indexOf('Start Date') != -1 || title.indexOf('Closure Date') != -1)
					{
						if(new_value.trim() != '' && new_value.trim() != '-')
						new_value = convertDate(new_value);
						else
						new_value = "-";
						if(old_value.trim() != '' && old_value.trim() != "-")
						{							
							old_value = convertDate(old_value);
						}
						else
						old_value = "-";
					}
