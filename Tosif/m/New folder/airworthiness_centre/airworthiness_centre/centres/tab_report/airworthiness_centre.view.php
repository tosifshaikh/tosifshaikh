...
while($db->next_record()){
       $tempdataArr[$db->f('client_id')][$db->f('work_status')][$db->f('component_id')] = $db->f('component_id');    
}

$wsids  = array();
$tempdataValArr = array();
foreach($tempdataArr as $tempkey=>$tempval){
	foreach($tempval as $key=>$val){
		$tempdataValArr[$tempkey][$key] = count($val);
		$wsids[$key]=$key;
	}
}
...
$db->getWorkStatus($whr,1,4,$wsids);
$statusArr = array();
while($db->next_record()){
   $statusArr[$db->f("id")] = array('name'=>$db->f('status_name'),'bg_color'=>$db->f('bg_color'),'font_color'=>$db->f("font_color"));   
  
   if(in_array($db->f("id"),array_keys($tempdataValArr[$db->f("client_id")]))){
	  $dataArr[$db->f("client_id")][$db->f("id")] = $tempdataValArr[$db->f("client_id")][$db->f("id")];
   }
}
  ...
  <tr>
  	<td align="left">
    	<table align="center" border="0" width="100%" cellpadding="0" cellspacing="0">
            ...
            <tr>
            <td align="left" colspan="2"><strong>Select Client:</strong>
			<select id="clientId" name="clientId" class="selectauto" onchange="getSelData(this.value);">
