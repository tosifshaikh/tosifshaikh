...
function loadGrid()
{
	try{
	    ...
	    var param = "section="+sectionVal+"&sub_section="+sub_sectionVal+"&act=GRID&type="+type+"&master_flag="+master_flag+'&client_id='+client_id;	
		param = param+"&comp_main_id="+comp_main_id+"&comp_id="+comp_id+"&template_id="+template_id;
...
function getAuditObj()
{
	...
	tempAudit={"user_id":UserID,"user_name":user_name,"section":$("#sectionVal").val(),"sub_section":$("#sub_sectionVal").val(),"type":$("#type").val(),"client_id":$("#clientVal").val(),
				"comp_main_id":$("#comp_main_id").val(),"tail_id":$("#comp_id").val()};	
	return tempAudit;
}
...
function getGridData()
{
	...
	for(var a in clientObj){
		...
		for(var c in dataObj[clVal]){
			var columnId_temp= c.replace("_","");
			if(dispObj[columnId_temp] && Object.keys(dispObj[columnId_temp]).length>0){
				cntRow=1;
				break;
			} else {
				cntRow=0;	
		...
		if(cntRow>0 && lovObj!=""){			
			...
			for(var b in dataObj[clVal]){
				...
				if(dispObj[columnId] && Object.keys(dispObj[columnId]).length>0){
...
function Open_Audit()
{
	...
	var lovAudit = window.open('airworthiness_centre_audit.php?section='+section+'&sub_section='+sub_section+'&type='+type+addStr,'lovAudit',
								 'height='+screenH+',width='+screenW+',scrollbars=yes,left=50,resizable=1,fullscreen=yes');
	lovAudit.focus();
}

function loadparentWin()
{
	var	section= $("#sectionVal").val();
	if(window.opener && section==3){
