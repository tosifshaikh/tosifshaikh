...
var excludeFilterArr= {1:new Array("Aircraft Type"),2:new Array("Engine Type")};
var tdIdStrArr = {2:"Engine"};
var compIdArr = {1:"aircraft_type",2:"engine_type"};
var colIdArr = {2:"ENGINETYPE"};
var ExpDateValObj = new Object();
...
mainHeaderObj= {1:{
	...
	mainCoumnHeaderObj={
					   ...
					   "Tail":{"col_id":"tailName",'elemId':'txtTail'},"MSN Number":{"col_id":"tailMSNO","elemId":"tailMSN"},"Manufacturer":{"col_id":"MANUFACTURER","elemId":"manufracutre"},
						...
						"Country of Registration":{"col_id":"REGCOUNTRY","elemId":"reg_country"},"Owner/Lessor":{"col_id":"OWNER","elemId":"owner"},"Last Review Date":{"col_id":"fl_ls_date","elemId":"date_lr"},"Next Review Date":{		                        "col_id":"fl_ns_date","elemId":"date_nr"},"Work Status":{"col_id":"fl_work_status","elemId":"comp_status"}
						},
						2:{
...
function renderGrid()
{
	try{
		...
		for(w in dataObj['data']){
			...
			var airTypeVal = w.replace("_","");
			mainTable+='<td colspan="'+colSpanLen+'" align="left"><a href="javascript: setHideVal('+t+');"><img width="9" height="9" border="0" id="ImgTypeId_'+t+'" src="'+addPath+'images/minus_active1.jpg"></a>&nbsp;'+airTypeVal+'</td></tr>';
			k++;
			var innerTable = '';
				...
				if(flg==0){
					...
					for(y in ColumnHeaderObj){
							...
							if($.inArray(y,excludeColumnArr)<0){							
								...
								var addStyle = "";														
								if(y=="Work Status"){															
									if(statusObj[tdVal] && tdVal!=""){
										 var bg_color = statusObj[tdVal]['bg_color'];
										 tdVal = statusObj[tdVal]['status_name'];									 
									} else {									
										 var bg_color ="#B6B6B4";
										 tdVal = "No Airworthiness Reviews Added";
									}
									addStyle = " style='background-color:"+bg_color+";'";
								}
								innerTable+='<td '+addStyle+'>'+tdVal+'</td>';
							}
						}
					}
					var comp_id = dataObj['data'][w][q]['component_id'];
					var client_id = dataObj['data'][w][q]['Client_id'];
					innerTable+= '<td width="3%" align="center"><a onclick="Fun_Open('+comp_id+','+client_id+')"><div class="view_active_icon"></div></a></td>';
					innerTable+='</tr>';
					k++;						
				}
			}
	...
	for(x in ColumnHeaderObj){
		if($.inArray(x,excludeColumnArr)<0){
			...
			if($.inArray(x,excludeFilterArr[type])>=0){	
				...
				filterTable+='<td class="tableCotentTopBackground" align="left"><input type="text" size="8" class="gridinput" name="filter_'+filterId+'" id="filter_'+filterId+'" value="'+filterValue+'" ';
				if(UserLevel==5){
...
function Fun_Open(compid,client_id)
{
	var type = $("#type").val();
	var param = '';
	if($("#inboxmod").val()==1)
		param = '&UID='+$("#UID").val()+'&inboxmod';
	var AirWin=window.open("airworthiness_centre.php?section=2&sub_section=1&type="+type+"&comp_id="+compid+"&client_id="+client_id+""+param,"AirWin",'height='+screenH+',width='+screenW+',scrollbars=yes,left=50,resizable=1,fullscreen=yes');
	AirWin.focus();
}
...
function openTabReport()
{
	...
	var airWorthiReport = window.open('airworthiness_centre.php?section='+section+'&type='+type+'&sub_section=5&master_flag=0',
						   'airWorthiReport','height='+screenH+',width='+screenW+',scrollbars=yes,left=50,resizable=1,fullscreen=yes')
	airWorthiReport.focus();	
	
}
function openEmailTemplate()
{	
	var tempairWorthi=window.open('manage_internalfiles_rows.php?sec=airworthiCenter&section=mailTemplate&TabId=0','tempairWorthi',"height="+screenH+",width="+screenW+",scrollbars=yes,resizable=1,fullscreen=yes");
	tempairWorthi.focus();
	
}
function openARCSequence()
{	
	var type = $("#type").val();
	var section = $("#sectionVal").val();
	
	var ARCSequence = window.open('airworthiness_centre.php?section='+section+'&type='+type+'&sub_section=6','ARCSequence','height='+screenH+',width='+screenW+',scrollbars=yes,left=50,resizable=1,fullscreen=yes');
	ARCSequence.focus();		
}
