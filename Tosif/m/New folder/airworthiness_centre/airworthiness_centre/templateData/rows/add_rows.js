...
function validateForm()
{
	...
	for(var i=0;i<parseInt(rowstoadd);i++){
		...
		for(hid in headerObj){
			...
			if($("#field_"+i+"_"+headerObj[hid]['column_id']) && $("#field_"+i+"_"+headerObj[hid]['column_id']).val()!="" 
				...
				if(is_reminder==2 || is_reminder==1){	
	...
	if(flgSubmit==0){
		flgSubmit=1;
		InsVal={"AuditVal":AuditObj,"dataVal":InsertObj};
...
function getSubTempByClient(clientId)
{
	if(clientId!=0)
	{
		document.getElementById("addRowTable").innerHTML = '';
		xajax_getSubTempByClient(clientId,document.getElementById("Type").value);
	}
	else
	{
		var strToTemplate = '<select onchange="GetTemplate(this.value);" tabindex="2" id="ddl_template" name="ddl_template"><option value="0">[Select Template]</option></select>';
		document.getElementById("getSubTempDDL").innerHTML = strToTemplate;
		document.getElementById("addRowTable").innerHTML = '';
		document.getElementById("addRowTable").innerHTML = '';
		document.getElementById("saveBtn").className = "disbutton";
...
function GetTemplate(val)
{
	...
	var strURL= 'airworthiness_centre.php';
	var req = getXMLHTTP();
	if (req)
	{
		req.onreadystatechange = function() 
		{
			if (req.readyState == 4)
			{
				if (req.status == 200) 
				{
					...
					renderSubTemplateGrid();
				}
			}
		}
		var section = document.getElementById("sectionVal").value;
			var sub_section = document.getElementById("sub_sectionVal").value;
			var link_id = document.getElementById("link_id").value;
			
		params="template_id="+val+"&act=GRID&add_section=ADD_DB_TEMP&ClientID="+document.getElementById('selclient').value+"&Type="+document.getElementById('Type').value+"&tabId="+document.getElementById('tab_id').value;
		params+="&section="+section+"&sub_section="+sub_section+'&link_id='+link_id;
		req.open("POST", strURL+'?t='+(new Date()), false);
...
function renderSubTemplateGrid()
{
		var table='';
		Obj=eval('('+Obj+')')
		
		var current_order=parseInt(document.getElementById('currentOrder').value);
		table+='<table cellspacing="1" border=0  cellpadding="3" class="tableContentBG" width="100%">';
		table +='<tr >';
		
		table +='<td align="left" class="tableCotentTopBackground">ItemId</td>';
		table +='<td align="left" class="tableCotentTopBackground">Description</td>';
		table +='<td align="left" class="tableCotentTopBackground">Hide From Third Party</td>';
		table +='<td align="left" class="tableCotentTopBackground">Hyperlink Option</td>';
		table +='<td align="left" class="tableCotentTopBackground">Hyperlink Value</td>';               
		table +='<td align="left" class="tableCotentTopBackground">Template Name</td>';
		table +='<td align="left" class="tableCotentTopBackground">Read Only</td>';
		table +='<td align="left" class="tableCotentTopBackground">Status</td>';
		table +='<td align="left" class="tableCotentTopBackground">Manufacturer</td>';
		table +='</tr>';
		innerTable = '';
		for(x in Obj)
		{
			for(y in Obj[x])
			{
				...
				innerTable +='<tr style="background-color:'+bgcolor+'">';
				//var itemChar=String.fromCharCode('A'.charCodeAt(0) + parseInt(y)+(current_order));
				var itemChar = convertToAlphabateScheme(parseInt(y)+(current_order+1));
				var itemStr=Item+itemChar;
				innerTable +='<td >';
				innerTable +=itemStr;
				innerTable +='</td>';
				var delCell= Obj[x][y].split('##STEMP##');
				for(var i=0;i<delCell.length;i++)
				{
					innerTable +='<td >';
					innerTable +=delCell[i];
					innerTable +='</td>';
				}
				innerTable +='</tr>';
			}
		...
		{
			table +='<tr >';
			table +='<td colspan = "9" align="center"> <strong>No Record(s) Found.</strong></td>';
			table +='</tr>';
			document.getElementById("saveBtn").className = "disbutton";
			document.getElementById("saveBtn").disabled = true;
		}
		else
		{
			...
			document.getElementById("saveBtn").className = "button";
			document.getElementById("saveBtn").disabled = false;
		}
	document.getElementById('addRowTable').innerHTML=table;
}
...
function SaveSubTemplate()
{
	...
	if(document.getElementById("ddl_template").value!=0 && document.getElementById("ddl_template").value!='')
	{
		xajax_SaveSubTemplate(xajax.getFormValues('addrowform'),Item)
	}
	else
	{
		alert("Please select a Sub Template.");
		document.getElementById("ddl_template").focus();
		return false;
	}
}

function convertToAlphabateScheme(number)
{
