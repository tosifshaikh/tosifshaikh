...
var ObjElement = {0:"",1:1,2:{0:"None",1:"Free Text",2:"List of Values",3:"Date Picker",4:"Reference Type"},3:1,4:{0:"No",1:"Yes"},5:{0:"No",1:"Yes"},6:""};
...
function getRows(dataId,clNum)
{
	...
	if(dataObj[dataId]){
		var flag=0;
			...
			if(valObj["is_reminder"]!=0){
				is_reminderVal[cl_id] = valObj["is_reminder"];
			}
...
function fnEdit()
{
	...
	enableForm();
	$("#clientVal").attr("disabled","disabled");
}
...
function fnSave()
{
	...
	if(filter_type==3){
		if($("#is_expiry").val() == 1){
			if(is_reminderVal[clientVal] && is_reminderVal[clientVal]!=0){
				alert("The expiry date functionlity can only be activated for one column within the status list");
				return false;		
			}
	...
	if(is_remVal!=0){		
		...
		for(var w in expiryAddObj){
			...
			insertObj['ExpiryVal'][w]= {"client_id":clientVal,"header_name":expiryAddObj[w],"filter_type":filter_type,"filter_auto":$("#filter_auto").val(),
					"master_flag":$("#master_flag").val(),"type":$("#type").val(),"read_only":$("#read_only").val(),"is_reminder":0,"view_flag":view_flag};
		}
	}
	
	insertObj[0]= {"client_id":clientVal,"header_name":field_val,"filter_type":filter_type,"filter_auto":$("#filter_auto").val(),
				"master_flag":$("#master_flag").val(),"type":$("#type").val(),"read_only":$("#read_only").val(),"is_reminder":is_remVal,"refMax_value":tempNumAdd};
	...
	for(var w in mainInsobj){
		...
		if(oldDataobj[w] && oldDataobj[w]!=mainInsobj[w]){			
			...
			if(w=="header_name"){				
				...
				if(oldDataobj["filter_type"]==4){
					var tempHeader = oldDataobj["header_name"].split("RPL_COMMA");
					var refmaxVal = "";
					if(tempHeader[0])
					headerName =$.trim(tempHeader[0]);
				...
				if(mainInsobj["filter_type"]==4){
					var tempNewHeader = mainInsobj["header_name"].split("RPL_COMMA");					
					if(tempNewHeader[0])
				...
				if(refOldmaxVal!=refnewmaxVal){					
					...
					auditObj[k]["old_value"] = refOldmaxVal;
					auditObj[k]["new_value"] = refnewmaxVal;
				}
				...
				auditObj[k]["field_title"] = changeColumn[w];
				auditObj[k]["old_value"] = oldDataobj["header_name"]+"&nbsp;&raquo;&nbsp;"+ObjElement[2][oldDataobj["filter_type"]];
				auditObj[k]["new_value"] = oldDataobj["header_name"]+"&nbsp;&raquo;&nbsp;"+ObjElement[2][mainInsobj["filter_type"]];
			}
...
function fnDelete()
{
	if(confirm("Are sure want to delete this record?")){
		...
		AuditObj["old_value"] =dataObj[mainRowid]["header_name"];
...
function fnOpenLovTab()
{
	...
	var airworthireorder = window.open('airworthiness_centre.php?section='+section+'&type='+type+'&sub_section=3',
...
function open_parent(flg)
{
	...
	for(var j in tabObj){
		...
		window.open('airworthiness_centre.php?section='+section+'&sub_section='+subSectionVal+'&type='+type+'&master_flag='+master_flag,
					 '_self','height='+screenH+',width='+screenW+',scrollbars=yes,resizable=yes');
	}
