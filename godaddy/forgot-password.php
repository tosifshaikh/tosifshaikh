...
if (!empty($_POST))
{
	$Email = (isset($_POST["Email"]) && $_POST["Email"] != "") ? $_POST["Email"] : '';
		...
		$strUserCheck = "SELECT * FROM `customer` WHERE email = '". mysqli_real_escape_string($con,$Email) ."' and authenticated = 1 "; 
		$tarray = mysqli_query($con,$strUserCheck);
		$tot = mysqli_fetch_array($tarray);
		if(!empty($tot))
		{
			$custpass = $tot["custpass"];
			...
			$body .= "<tr><td style='height:30px;'>Dear ".$tot["FullName"].",</td></tr>";
			$body .= "<tr><td style='height:30px;'>This email was sent by ".$SiteName." in response to your request to recover your password.</td></tr>";
			$body .= "<tr><td style='height:30px;'>Your Account details are as below:</td></tr>";
			$body .= "<tr><td style='height:30px;'>Username :<strong>". $tot["email"] ."</strong></td></tr>";
			$body .= "<tr><td style='height:30px;'>Password :<strong>". $custpass ."</strong></td></tr>";
			$body .= "<tr><td style='height:30px;'>This request was made from IP address :<strong>". GetIPAddress() ."</strong></td></tr>";
			...
			SendMail($AdminMailAddress,$SiteName,$tot["email"],$tot["FullName"],$subject,$body);

			header( 'Location:forgot-password-success.php') ;
		}
		else{
			$ErrMsg = "<p><strong>The email address you have entered is not registered with us.<br/>Please Try again</strong></p>";
		}
}
else if(!empty($_GET)){
