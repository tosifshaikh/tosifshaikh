                ...
                <form action="customer-order-proc.php" method="POST" >              
                	<div class="row selectplace">
                        <div class="col-sm-6 pickup">
                            <h5>Pickup From</h5>
                           <!--  <select id="zipcode" name="zipcode" class="form-control" onChange="getZip(this)"  placeholder="Zip Code" aria-label="Zip Code" title="Select zipcode" style="margin-top:9px" required></select>-->
                            <select id="Store" name="Store" class="form-control" onChange="getAddress(this)"  placeholder="Store Name" aria-label="Enter Store Name" title="Enter Store Name" style="margin-top:9px" required>
                            <option value="">Select Store Name</option>
                            </select>
                              <!--<input type="text" class="form-control" required id="Store" name="Store" />-->
                              <textarea class="form-control" id="SAddress" name="SAddress" title="Enter Store Address" rows="6" required placeholder="Store Address" aria-label="Enter Store Address"  style="margin-top:11px" ></textarea>
                        ...
                        <div class="col-sm-6 delivery">
                            <h5>Select Your Address</h5>
                              ...
                              <textarea class="form-control" id="CAddress" title="Write New Address" name="CAddress" rows="6" required placeholder="Write New Address" aria-label="Write New Address"></textarea>
                    ...
                    <div class="row selcttime">  
		                <h5>Select Delivery Slots</h5>
                         <div class="col-sm-4">
                         <input type="text" id="dateSlot" name="dateSlot" class="form-control" placeholder="Select Date" required>
                         </div>                             
                        <div class="col-sm-4">
                        <select id="timeSlot" class="form-control" name="timeSlot"  required></select>                                                                                      
                    </div>

                    <div class="col-sm-4">
                      <input type="text" name="invoiceNumber" id="invoiceNumber" placeholder="Enter Invoice No" class="form-control" aria-label="Enter nvoice No" required>
                        	</div>
                        </div>
                         <div class="row">
                    	<div class="col-sm-12">
                        	<textarea class="form-control" id="Comments" name="Comments" title="Enter Comments" rows="6" placeholder="Enter Comments (Provide us your Order)" aria-label="Enter Comments" style="margin-top:6px"></textarea>
                        </div>
                    </div>               
                    <div class="row">
              ...
              <div class="tab-pane <?php echo $pagesel3;?>" id="uraddress">
				 ...
				 <form action="add-my-address-proc.php" method="Post">
                	<div class="row uraddresses">
                        <div class="col-sm-6">
                            <h5>Address 1</h5>
                             <textarea class="form-control" id="Address1" name="Address1" title="Enter Address" rows="6" required placeholder="Enter Address" aria-label="Enter Address"><?php echo $custAdd1; ?></textarea>
...
<script>
var storeObj=eval(<?php echo  json_encode($storeArr)?>);
var deliverySlot=["9:00 am - 10:00 am","10:00 am - 11:00 am","11:00 am - 12:00 pm","12:00 pm - 1:00 pm","1:00 pm - 2:00 pm","2:00 pm - 3:00 pm","3:00 pm - 4:00 pm","4:00 pm - 5:00 pm","5:00 pm - 6:00 pm","6:00 pm - 7:00 pm"];
...
function getStoreDropdown()
{
		...
		for(o in storeObj)
		{
			...
			{
			pinVal=o.replace("_","");
			 sel.options[sel.options.length]=new Option(storeObj[o][s].storename+"-"+pinVal ,s)
			}
		}
...
function getAddress(ele)
{
	...
	if(ele.value!=''){
		for(o in storeObj){
			if(storeObj[o][ele.value]){
				document.getElementById('SAddress').innerHTML=storeObj[o][ele.value].storeaddress;
			}
		}
	}
...
function GetDeliveryslots()
{
	...
	sel.options[sel.options.length]=new Option("Select TimeSlot","");
	for(var i=0,len=deliverySlot.length;i<len;i++ )
	{
		sel.options[sel.options.length]=new Option(deliverySlot[i],deliverySlot[i]);
	}
}
