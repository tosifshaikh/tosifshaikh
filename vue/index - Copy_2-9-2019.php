...
<td><!--<input v-model="search" placeholder="Search" v-on:key="enter"></td>-->
  ...
  <input  v-if="colval.filtertype==1" :key="colval.filterid" v-model.lazy.trim="colval.filterVal" placeholder="Search" v-on:keydown="addFilter(colval,$event)"  >
...
<!--<tr v-for="(item, index) in filtered " :key='index'  v-on:click='tes88(index,$event,contents)' 	 @dragstart="dragStart(index,$event)"   @dragover.prevent @dragenter="dragEnter" @dragleave="dragLeave" @dragend="dragEnd" @drop='drop'> -->

<tr v-for="(item, index) in filtered " :key='index'   v-on:click='tes88(index,$event)' 	draggable="true"  @mousedown="mouseDown(index)" @mouseup="mouseUp(index)"   @mouseover="mouseOver(index)" @mouseout="mouseOut(index)"  v-bind:style="(dragStyleObj!=null && dragStyleObj[index]!=null)?dragStyleObj[index]:'' " >
...
<label v-if = "!copyObject[index].edit && cols.inputType!=3"> {{item[colkey]}} </label>
 <input  v-if = "copyObject[index].edit && cols.inputType==1" 
	 ...
	 <textarea  v-if = "copyObject[index].edit && cols.inputType==2" 
      @blur= "editTodo(index,colkey,$event);" :ref='index' :id="index+'_'+colkey"  v-model.lazy.trim="item[colkey]"  v-bind:class="{dirty:(validateobj!=null && validateobj[index] && validateobj[index].includes(colkey)==true)?true:false}">
	  </textarea>
...
var mainVue=Vue.extend({
	...
	contents:[{ company_name: 'ABC Infotech', city: 'New Jersey', turn_over: 100,status:1,'edit':false,display_order:3 },
        { company_name: 'Amazon Web Services', city: 'New York', turn_over: 400,status:2,'edit':false,display_order:1 },
        { company_name: 'Digital Ocean', city: 'Washington', turn_over: 200,status:3,'edit':false,display_order:0 },
		{ company_name: 'Digital Ocean1', city: 'Washington1', turn_over: 2020,status:4,'edit':false,display_order:2 }
		],
   ...
   ,resetArr:[],copyObject:[],items:[],dragStyleObj:{},downIndex:-2
 ...
 new mainVue({
  el: '#app'
   ...
   ,
    mouseDown(ind){
	  ...
	  this.$set(this.dragStyleObj,ind,{
                    cursor:'move'
		 //  cursor:'move',height:'40px !important', outline:'solid #f00',
	      // background:'#FFCC99'
	   });
	   this.downIndex=ind;
	  // if(!this.dragStyleObj && !this.dragStyleObj[ind])
	   {
		   /* this.dragStyleObj[ind]={};
	   this.dragStyleObj[ind]={
		   outline:'solid #f00',
		   cursor:'move',
		   height:'40px !important',
	   ...
	   };this code will not reflect any changes at run time
	   this.$set(this.dragStyleObj,ind,{
		   outline:'solid #f00',
		   cursor:'move',
		   height:'40px !important',
   ...
   ,
   mouseOver(ind){
	   console.log('mouseOver',ind,this.downIndex,this.dragStyleObj);
	   if(this.downIndex!=-2)
	   {
	   this.$set(this.dragStyleObj,ind,{
   ...
   ,
   mouseOut(ind){
	   console.log('mouseOut',ind,this.dragStyleObj);
	    if(this.downIndex!=-2){
	    this.$set(this.dragStyleObj,ind,{
   ...
   mouseUp(ind){
	  // ev.preventDefault();
	    console.log('mouseUp',ind,this.dragStyleObj, this.downIndex+' this.downIndex');
		
                if(this.downIndex!=ind)
                {
		...
		//this.dragStyleObj={
		 //  this.$set(this.dragStyleObj,ind,{
		  // cursor:'move',height:'40px !important'
	  // });
   },
   drop(){
	    console.log('drop');  
